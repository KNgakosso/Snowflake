...
class Mesh:
    ...
    def __init__(self, nb_rings : int = 2):
        ...
        if nb_rings == 0:
            ...
            #sommet
            for side in range(6):
                id = (nb_rings, nb_rings * side)
                neighbors = {(nb_rings, (nb_rings * side + 1) % (nb_rings * 6)),
                                (nb_rings, (nb_rings * side - 1) % (nb_rings * 6)),
                                (nb_rings - 1, (nb_rings - 1) * side)
                }
            ...
            for side in range(6):
                for i in range(1, nb_rings):
                    id = (nb_rings, nb_rings * side + i)
                    neighbors = {(nb_rings, (nb_rings * side + i + 1) % (6 * nb_rings)),
                                    (nb_rings, (nb_rings * side + i - 1) % (6 * nb_rings)),
                                    (nb_rings - 1, ((nb_rings - 1) * side + i) % (6 * (nb_rings - 1))),
                                    (nb_rings - 1, ((nb_rings - 1) * side + i -1) % (6 * (nb_rings -1)))
                    }
                    dict_cells[id] = Cell(id, neighbors)
